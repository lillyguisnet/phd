...
def _get_all_masks_for_object(args):
    """Helper function for multiprocessing pool to collect all masks for a single object."""
    obj_id, segments_for_object, frame_indices, mask_shape = args
    # Pre-allocate a numpy array for efficiency
    masks = np.zeros((len(frame_indices),) + mask_shape, dtype=bool)
    for i, frame_idx in enumerate(frame_indices):
        # Get the mask for the specific object ID from the pre-filtered dictionary
        mask = segments_for_object.get(frame_idx)
        if mask is not None:
            masks[i] = mask
    return obj_id, masks
...
def save_video_segments_to_h5(video_segments, video_dir, output_dir, frame_mapping, num_workers=None):
    ...
    Preserves frame indices in the saved data.
    """
    ...
    # Reorganize data for more efficient parallel processing
    segments_by_object = {}
    for frame_idx, segments in filtered_video_segments.items():
        for obj_id, mask in segments.items():
            if obj_id not in segments_by_object:
                segments_by_object[obj_id] = {}
            segments_by_object[obj_id][frame_idx] = mask
            
    object_ids = sorted([oid for oid in segments_by_object.keys() if oid is not None])
    if None in segments_by_object:
    ...
    sorted_frame_indices = sorted(filtered_video_segments.keys())
    ...
    with h5py.File(output_path, 'w') as f:
        ...
        # Save frame indices
        f.create_dataset('frame_indices', data=sorted_frame_indices, dtype=np.int32)

        # Create datasets for each object's masks
        ...
        # Create tasks with only the data needed for each object
        tasks = [(obj_id, segments_by_object[obj_id], sorted_frame_indices, mask_shape) for obj_id in object_ids]

        with multiprocessing.Pool(processes=num_workers) as pool:
            results = tqdm.tqdm(pool.imap_unordered(_get_all_masks_for_object, tasks), total=len(tasks), desc="Processing objects")
            for obj_id, masks_array in results:
                obj_id_str = str(obj_id) if obj_id is not None else 'None'
                f[f'masks/{obj_id_str}'][:] = masks_array
...
def load_video_segments_from_h5(h5_path):
    """
    Load video segments from HDF5 file and reconstruct the original video_segments structure.
    
    Args:
        h5_path (str): Path to the HDF5 file containing the video segments
        
    Returns:
        dict: Reconstructed video_segments dictionary with frame indices as keys
    """
    video_segments = {}
    ...
    with h5py.File(h5_path, 'r') as f:
        ...
        # Get object IDs from attributes
        object_ids = [int(obj_id) if obj_id != 'None' else None 
                     for obj_id in f.attrs['object_ids']]
        
        # For each frame index, reconstruct the frame's data
        for frame_idx in frame_indices:
            frame_data = {}
            for obj_id in object_ids:
                obj_id_str = str(obj_id) if obj_id is not None else 'None'
                # Get the mask for this object at this frame
                mask = f[f'masks/{obj_id_str}'][frame_idx]
                frame_data[obj_id] = mask
            video_segments[frame_idx] = frame_data
    
    print(f"Loaded video segments from: {h5_path}")
    print(f"Number of frames loaded: {len(video_segments)}")
    print(f"Object IDs present: {sorted([obj for obj in object_ids if obj is not None])}")
