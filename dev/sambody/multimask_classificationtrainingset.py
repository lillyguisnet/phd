...
"""
For each image:
    Get cropped image of each mask,
        except those that touch edge a lot.
...
for png in range(len(mm)):
    ...
    for maskpred in range(len(masks)):
        ...
        if is_on_edge(*bbox, h, w) == True:
            continue
        
        x, y, w, h = bbox
        cropped_image = img[y:y+h, x:x+w]
        savename = "m" + str(maskpred) + "_" + mm[png].split(os.sep)[-1]
        saveloc = "/home/maxime/prg/phd/dev/sambody/multimask_crops/" + savename
...
for png in range(len(mm)):
    ...
    for maskpred in range(len(masks)):
        ...
        savename = "c" + str(maskpred) + "_" + mm[png].split(os.sep)[-1]
        saveloc = "/home/maxime/prg/phd/dev/sambody/multimask_cutouts/" + savename
